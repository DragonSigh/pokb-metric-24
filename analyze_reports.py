...
df_pass_dvn['Подразделение'] = df_pass_dvn['Структурное подразделение'].apply(lambda x: re.search('ОСП \d', x)[0] if re.match('^ОСП \d.*$', x) else 'Ленинградская 9')
...
df_agg = df_pass_dvn \
    .merge(df_disp_tmk,  
           ...
           how='outer',
           indicator=True) \
    .drop(['#_x', '#_y', 'Медицинская организация диспансеризации', 'ОГРН', 'ID подразделения_x', 'ID подразделения_y',
           'Причина закрытия', 'Процент прохождения', 'Вид обследования', 'Статус актуальный', 'Дата обновления статуса',
           'Текст сообщения', 'Группа здоровья', 'Результат обращения', 'Период', 'Наименование медицинской организации',
           'ОГРН медицинской организации', 'Дата последнего мероприятия 1 этапа диспансеризации', 'Дата рождения пациента',
            'Дата создания карты диспансеризации'], axis=1) \
    ...
    .count().reset_index() \
    .drop(['Отделение', 'Врач', 'ФИО пациента', 'Номер МКАБ', 'Дата закрытия диспансеризации'], axis=1)
...
save_to_excel(df_agg, reports_path + '\\Показатель 24\\agg_24.xlsx')